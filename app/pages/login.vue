...
async function handleSubmit() {
  ...
  try {
    ...
    error.value = fetchErr?.data?.message || 'Invalid email or password'
  } finally {
    loading.value = false
  }
}
        ...
        <form class="space-y-5" @submit.prevent="handleSubmit">
          ...
          <div>
            <div class="flex items-center justify-between">
              ...
              <NuxtLink to="/forgot-password" class="text-xs font-medium text-primary-600 hover:text-primary-500 dark:text-primary-400">
                Forgot password?
              </NuxtLink>
            </div>
            <input
